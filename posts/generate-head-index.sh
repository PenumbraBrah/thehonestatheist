...
lmeta=( $(ls [^_]*.json | sort -r | head) )
lpost=( $(ls *.md | sort -r | head) )
older=( $(ls [^_]*.json | sort | head -n -10) )
older+=( $(ls *.md | sort | head -n -10) )

# move oldest posts to archive
printf '%s\0' "${older[@]}" | xargs -0 -I {} mv {} ../archive/posts/ 2> /dev/null

rm -f ../_body.jade 2> /dev/null
...
do
	...
	if [ $incr -eq 1 ] ; then
		...
		if [[ ${posts[$incr - 1]} -le 5 && $post -le 5 ]] ; then
			lock[$incr - 1]=4
			lock[$incr]=4
			new_row=true
		else
...
#echo "!= partial(\"_header\")" >> head.jade
...
#echo "!= partial(\"_footer\")" >> head.jade

mv head.jade ../_body.jade
